...
public class Castle extends Figure {
	...
	public boolean move(Position origin, Position destination) {
        ...
        ((origin.getRow() != destination.getRow()) || (origin.getColumn() != destination.getColumn())) &&
        // Inside the bounds
        ((origin.getRow() >= 1 && origin.getRow() <= 8) && (origin.getColumn() >= 1 && origin.getColumn() <= 8)
        		&& (destination.getRow() >= 1 && destination.getRow() <= 8) && (destination.getColumn() >= 1 
        		&& destination.getColumn() <= 8));
	}
}
